...
- The **latest** single-run report from `/review` or `/review-branch`: `Output/CodeReview/AUDIT_REPORT_*.json` (timestamped; excludes `*_MERGED.json`). Use `/fix-merged` when you want to fix issues from a merged parallel run instead.
...
raise SystemExit("No audit report JSON found under Output/CodeReview/ (use /fix-merged for merged reports)")
PY
```

2. Open and parse the JSON file path printed above.
3. Extract:
   - The audit `verdict`
...
4. **Linear requirements context (MANDATORY — do not silently skip)**:
...
- **Input**: `<path to latest Output/CodeReview/AUDIT_REPORT_*.json>`
- **Issues total**: X
- **Confirmed & fixed**: A
